_G.FormatSelectionAndWrap = function()
	...
	local _, start_pos = pcall(vim.api.nvim_buf_get_mark, 0, "<")
	local _, end_pos = pcall(vim.api.nvim_buf_get_mark, 0, ">")
	...
	if mode == "V" or (mode == "v" and ec == 2147483647) then
		...
		-- 字符模式：sc 直接使用，ec 转为排除
		ec = math.min(ec + 1, line_length)
	end

	local lines = vim.api.nvim_buf_get_text(0, sr, sc, er, ec, {})
	-- 3. 基于 lines 数组直接分组：空行分隔段落，连续非空行为一个段落
	local paragraphs = {}
	local current = {}
	...
	for _, line in ipairs(lines) do
		if line:match("^%s*$") then
			if #current > 0 then
				table.insert(paragraphs, { lines = current })
				current = {}
			end
			table.insert(paragraphs, { empty = true })
		else
	...
	for _, para in ipairs(paragraphs) do
		if para.lines then
			local text = table.concat(para.lines, " ")
			text = text:gsub("%s+", " ")

			-- 处理 CJK 字符：如果两个非 ASCII 字符之间有空格，则移除空格
			text = vim.fn.substitute(text, "\\v([^\\x00-\\xff])\\s+([^\\x00-\\xff])", "\\1\\2", "g")
			...
			if is_tex then
				...
				for _, cmd_pattern in ipairs(latex_spacing_cmds) do
					...
					text = text:gsub(search_pattern, "~%1")
				end
				-- ===============================================
				...
				while j <= #text do
					...
					if c == "\\" then
						...
						if j < #text then
							table.insert(chars, text:sub(j + 1, j + 1))
						end
						j = j + 2
					elseif c == "$" then
	...
	-- 6. 直接从段落列表重建行数组，写回 Buffer
	local new_lines = {}
	for _, para in ipairs(paragraphs) do
		if para.processed then
			table.insert(new_lines, para.processed)
		elseif para.empty then
